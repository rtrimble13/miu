...
| `miu validate`           | Reconstruct S&P 500 Health Care over 2015–2023 and compare to a bundled reference. Fails if annualized tracking error > 75 bps. |
...
All prices are FMP's dividend-adjusted series, so equal- and market-cap-weighted
results are effectively total-return.
...
When a held name delists during a period:

1. We hold its **last traded adjusted price** as the terminal value through
   the delisting date, then drop it at the next rebalance.
2. If `/mergers-acquisitions` indicates a **cash deal**, we apply the cash
   consideration as the terminal per-share value at the delisting date.
3. If a **stock-for-stock deal** where the acquirer is already in the
   index, we redistribute the position proportionally into the acquirer's
   weight; otherwise we fall back to the cash-equivalent.
...
**Why is `miu validate`'s tracking error not zero?**
The bundled reference series is generated with a methodologically similar but
not identical construction (monthly vs. quarterly rebalance, ten-name
universe). It exists as a canary: if a regression in the engine pushes the
tracking error above 75 bps annualized, the test fails.
